...
namespace ArmyArranger.Models
{
    class AddUnitsModel
    {
        ...
        public void AddUnit(string name, int nationID, string type, string composition, string weaponDescription, int armourClass, int inexperienced, int regular, int veteran, int pointsInexp, int pointsReg, int pointsVet, int baseSize, int maxSize, ObservableCollection<GameRule> rulesList, ObservableCollection<Weapon> weaponsList)
        {
            try
            {
                EmptyUnit.CreateNewAndSaveToDB(name, nationID, type, composition, weaponDescription, armourClass, inexperienced, regular, veteran, pointsInexp, pointsReg, pointsVet, baseSize, maxSize, rulesList, weaponsList);
            }
            catch (Exception ex)
            {
        ...
        public void UpdateUnit(Unit selectedUnit, string name, int nationID, string type, string composition, string weaponDescription, int armourClass, int inexperienced, int regular, int veteran, int pointsInexp, int pointsReg, int pointsVet, int baseSize, int maxSize, ObservableCollection<GameRule> rulesList, ObservableCollection<Weapon> weaponsList)
        {
            ...
            try
            {
                selectedUnit.UpdateInDB(name, nationID, type, composition, weaponDescription, armourClass, inexperienced, regular, veteran, pointsInexp, pointsReg, pointsVet, baseSize, maxSize, rulesList, weaponsList);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
            ...
            if (selectedUnit == null)
            {
                if (UnitWithThisName != null)
                {
                    if (PromptQuestion("Unit '" + name + "' alredy exist, do you want to override it?"))
                        UpdateUnit(UnitWithThisName, name, nationID, type, composition, weaponDescription, armourClass, inexperienced, regular, veteran, pointsInexp, pointsReg, pointsVet, baseSize, maxSize, selectedRulesList, selectedWeaponsList);
                }
                else
                {
